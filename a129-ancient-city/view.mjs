...
export class CityView{
  constructor(container){
    ...
    try{
      ...
      this.scene=new T.Scene();this.scene.background=new T.Color(0xebe7db);this.camera=new T.OrthographicCamera(-40,40,35,-35,.1,500);
      ...
      this.controls=new OrbitControls(this.camera,this.canvas);this.controls.enableDamping=true;this.controls.dampingFactor=.075;this.controls.minZoom=.55;this.controls.maxZoom=9;this.controls.maxPolarAngle=Math.PI*.46;this.controls.enablePan=true;this.controls.screenSpacePanning=true;
  ...
  fit(){if(!this.renderer)return;this.camera.zoom=1;this.controls.target.set(0,0,0);if(this.mode==='3d')this.camera.position.set(55,67,69);else this.camera.position.set(0,110,.01);this.camera.lookAt(0,0,0);this.controls.update();this.resize();}
  resize(){
    const w=Math.max(1,this.container.clientWidth),h=Math.max(1,this.container.clientHeight);
    if(this.renderer){this.renderer.setSize(w,h,false);const c=this.city,span=c?Math.max(c.height*1.03,c.width/(w/h)*(this.mode==='3d'?1.30:1.08)):60;this.camera.left=-span*w/h/2;this.camera.right=span*w/h/2;this.camera.top=span/2;this.camera.bottom=-span/2;this.camera.updateProjectionMatrix();}
